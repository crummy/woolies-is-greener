...
export async function searchWoolworths(query: string): Promise<SearchResult> {
  ...
  try {
    ...
    const nzResponse = await fetch(
      `${NZ_BASE_URL}/api/v1/products?target=search&search=${encodeURIComponent(
        query
    ...
    result.nz = nzData.products.items
      ...
      .filter((product): product is z.infer<typeof NZProductSchema> => product !== null);
  } catch (error) {
    result.error = `NZ search failed: ${error instanceof Error ? error.message : String(error)}`;
  }
  ...
  try {
    ...
    const abck = cookies?.split(', ').find((cookie) => cookie.startsWith('_abck'));
    if (!abck) {
      throw new Error('No _abck cookie found');
    }

    const auResponse = await fetch(`${AU_BASE_URL}/apis/ui/Search/products`, {
      method: 'POST',
    ...
    result.au = parsedResponse.Products.flatMap((group) => {
      ...
      }).filter((product): product is z.infer<typeof AUProductSchema> => product !== null);
    });
  } catch (error) {
