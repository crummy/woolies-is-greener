...
import { getProductsByBasketName } from '~/services/db'; // Import DB functions
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  // Use the DB service function without context
  const { allBaskets, selectedBasketId, productsInBasket } =
    await getProductsByBasketName(selectedBasketNameParam);

  let totalAuPrice = 0;
  let totalNzPrice = 0;
  for (const product of productsInBasket) {
    totalAuPrice += product.auPrice || 0;
    totalNzPrice += product.nzPrice || 0;
  }
...
export default function Index({ loaderData }: Route.ComponentProps) {
  const { allBaskets, selectedBasketId, productsInBasket, totalAuPrice, totalNzPrice, today } =
    loaderData;
    ...
    <div className="min-h-screen bg-yellow-100 p-6 font-sans sm:p-8">
      ...
      <section className="mb-10 flex flex-wrap justify-center gap-4 sm:mb-12 sm:gap-8">
        {sortedBaskets.map((basket) => (
          <Link
            ...
            className={`w-36 rounded-lg p-4 text-center shadow-md transition-all duration-150 sm:w-48 sm:p-6 ${getBasketStyle(basket.name)} ${basket.id === selectedBasketId ? 'scale-105 ring-4 ring-yellow-400 ring-offset-2' : 'ring-0'}`}
      ...
      {selectedBasketId && productsInBasket.length === 0 && (
        <div className="mx-auto mt-8 max-w-3xl rounded bg-white p-4 text-center text-gray-600 shadow-md">
          ...
          {allBaskets.find((b) => b.id === selectedBasketId)?.name || 'selected'}' basket.
        </div>
      )}
